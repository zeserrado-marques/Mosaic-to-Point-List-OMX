...
output = input + File.separator + "post_processing_analysis_single_file";
File.makeDirectory(output);

// path to tables
path_table_folder = input + File.separator + "centroids_table_single_file";
...
image_size = n_size_array[1] - (0.015 * n_size_array[1]); // 1.5 % of overlay between tiles
counter = 0;
...
setBatchMode("hide");
processMosaic(input_file, output, path_table_folder);
...
function processMosaic(input_file, output, path_table_folder) {
	...
	if (endsWith(input_file, ".dv")) {
		...
		Table.save(path_table_folder + File.separator + File.getNameWithoutExtension(input_file) + "_point_list.csv");
		// save processing log
		selectWindow("Log");
		saveAs("txt", output + File.separator + File.getNameWithoutExtension(input_file) + "_log_centroid.desktop");
